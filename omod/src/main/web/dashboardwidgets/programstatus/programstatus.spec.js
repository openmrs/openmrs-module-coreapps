...
describe('ProgramStatus', () => {
	...
	beforeEach(() => {
		...
		$httpBackend.expectGET('/ws/rest/v1/session').respond(sessionResponse);
		$httpBackend.expectGET('/ws/rest/v1/session?v=custom:(privileges)').respond(sessionResponse);
		$httpBackend.expectGET('/ws/rest/v1/appui/session?v=custom:name,uuid').respond(sessionResponse);

		$httpBackend.expectGET('/ws/rest/v1/location?tag=fb6f5fe0-904b-4a85-9806-d72ec139f9de&v=custom:display,uuid').respond(locationResponse);
		$httpBackend.expectGET('/ws/rest/v1/program/222e229b-f011-45c5-940e-e5599383c967?v=custom:display,uuid,outcomesConcept:(uuid),workflows:(uuid,concept:(display),states:(uuid,initial,terminal,concept:(display))').respond(programResponse);

		$httpBackend.expectGET('/ws/rest/v1/programenrollment?patient=919caaf7-122d-4001-8b53-ecdce347c8a5&v=custom:uuid,program:(uuid),dateEnrolled,dateCompleted,outcome:(display),location:(display,uuid),dateCompleted,outcome,states:(uuid,startDate,endDate,dateCreated,voided,state:(uuid,concept:(display)))').respond(programStatesResponse);
		//$httpBackend.expectGET('/ws/rest/v1/programenrollment?limit=20&patient=919caaf7-122d-4001-8b53-ecdce347c8a5&v=custom:program:(uuid,display),dateEnrolled,dateCompleted&voided=false').respond(programEnrollmentResponse);
    ...
    it('should sort first by start date, second by end date, and third on date created', () => {
		let bindings = {
						config: {
							...
							label:"icrcprograms.hcd.statusWidget.label",
